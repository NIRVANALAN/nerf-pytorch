...
def torchvisoin_single_gpu_test(model, data_loader, show=False, out_dir=None):
    """
    single_gpu_test for torchvision model, open-mmlab datasets
    """
    model.eval()
    results = []
...
def main():
    ...
    outputs = torchvisoin_single_gpu_test(model, data_loader)
    import ipdb

    ipdb.set_trace()

    torch.save(torch.stack(outputs, dim=0), "ImageNet_val_feats.pt")
